    ...
    <motion.div className={styles.card} layout>
      <motion.div className={styles.cardHeader} onClick={() => setIsOpen(!isOpen)} layout>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem', overflow: 'hidden' }}>
          <span className={styles.cardTitle} style={{ whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
            {item.type === 'promotion' ? '[프로모션] ' : '[이벤트] '}
            {item.title}
          </span>
        ...
        {isOpen && (
          ...
          >
            <div className={styles.cardBody}>
              <p className={styles.cardText} style={{ marginTop: '0.5rem' }}>{item.content}</p>
...
export default function MainTracker() {
  ...
  // Custom Confirm State for Deletion
  ...
  const handleCloseModal = () => {
    ...
    setTimeout(() => setPostToEdit(null), 300); // Wait for modal close animation
  };
  ...
  return (
    <div className={styles.container}>
      ...
      <div className={styles.tabs}>
        ...
        <button
          className={clsx(styles.tab, tab === "notice" && styles.tabActive)}
          onClick={() => setTab("notice")}
        >
          공지사항
        </button>
        <button
          className={clsx(styles.tab, tab === "promotion" && styles.tabActive)}
          onClick={() => setTab("promotion")}
        >
          프로모션
        </button>
        <button
          className={clsx(styles.tab, tab === "event" && styles.tabActive)}
      ...
      <div className={styles.content}>
        {isLoading ? (
          ...
          {filteredData.length > 0 ? (
            ...
            >
              해당 날짜에 진행 중인 {tab === "all" ? "게시물이" : tab === "notice" ? "공지사항이" : tab === "promotion" ? "프로모션이" : "이벤트가"} 없습니다.
            </motion.div>
          )}
        </AnimatePresence>
        )}
